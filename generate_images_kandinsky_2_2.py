...
def main():
    ...
    ckpt_id = "kandinsky-community/kandinsky-2-2-decoder"
    pipe = AutoPipelineForText2Image.from_pretrained(ckpt_id, torch_dtype=torch.float16).to(
        "cuda"
    )
    ...
    for i in range(0, len(drawbench), BATCH_SIZE):
        ...
        images = pipe(prompts, generator=generator, prior_guidance_scale=1.0, num_inference_steps=25).images
        for j in range(len(images)):
            img_name = f"kandinsky_v22_{i + j}.png"
        ...
        for j in range(len(images)):
            img_name = f"kandinsky_v22_upsampled_prompt_{i + j}.png"
            images[j].save(img_name)
